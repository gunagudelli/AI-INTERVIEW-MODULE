...
const SS: Record<string, { bg: string; color: string }> = {
  ...
  approved:       { bg: '#f5f3ff', color: '#6d28d9' },
  shortlisted:    { bg: '#f5f3ff', color: '#6d28d9' },
...
const RecruiterReferrals: React.FC = () => {
  ...
  const statCards = [
    ...
    { label: 'Approved',  value: stats?.approved         ?? 0, color: '#6d28d9' },
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: "'Inter', -apple-system, sans-serif" }}>
      <style>{`
        ...
        .rr-card  { transition: box-shadow .15s, border-color .15s, transform .15s; }
        .rr-card:hover  { box-shadow: 0 4px 16px rgba(79,70,229,.09) !important; transform: translateY(-1px); border-color: #c7d2fe !important; }
        .rr-row   { transition: background .1s; }
        .rr-row:hover td { background: #f5f3ff !important; }
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '24px 28px', display: 'flex', gap: 24, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {tab === 'analytics' ? (
              ...
              {!analytics ? (
                ...
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 10 }}>
                    {[
                      { label: 'Total Referrals', value: analytics.overview.total_referrals, color: '#4f46e5' },
                      { label: 'Approval Rate',   value: `${analytics.overview.approval_rate}%`, color: '#15803d' },
                      { label: 'Hire Rate',        value: `${analytics.overview.hire_rate}%`, color: '#0369a1' },
                        ...
                        {analytics.top_performers.length === 0
                          ...
                          : analytics.top_performers.map((p: any, i: number) => (
                            <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                              ...
                              <td style={{ padding: '11px 16px', color: '#4f46e5', fontWeight: 700 }}>{p.referrals}</td>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    {Object.entries(analytics.department_breakdown).map(([dept, count]: any) => (
                      <div key={dept} style={{ background: 'white', borderRadius: 10, border: '1px solid #e2e8f0', padding: '14px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <span style={{ fontSize: 20, fontWeight: 800, color: '#4f46e5' }}>{count}</span>
                      </div>
                    ))}
                  </div>
                </>
                  ...
                  {topReferrers.length === 0
                    ...
                    : topReferrers.map((r: any, i: number) => (
                      <tr key={r.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={{ padding: '12px 16px', fontWeight: 700, color: '#4f46e5' }}>{r.referral_count || r.total_referrals}</td>
            ...
            <>
              ...
              {/* Bulk Actions */}
              {selected.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#eef2ff', border: '1px solid #e0e7ff', borderRadius: 8, padding: '8px 14px', marginBottom: 12 }}>
                  <span style={{ fontSize: 13, color: '#4f46e5', fontWeight: 600 }}>{selected.length} selected</span>
                  ...
                  <button onClick={bulkUpdate}
                    style={{ padding: '5px 12px', background: '#4f46e5', color: 'white', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Update</button>
              ...
              <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                {loading ? (
                  <div style={{ padding: 48, textAlign: 'center', color: '#94a3b8' }}>
                    <div style={{ width: 28, height: 28, border: '3px solid #e2e8f0', borderTop: '3px solid #4f46e5', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
                    Loading referrals...
                  </div>
                ) : referrals.length === 0 ? (
                  <div style={{ padding: '60px 24px', textAlign: 'center' }}>
                    ...
                    {(statusFilter || search) && (
                      <button onClick={() => { setStatusFilter(''); setSearch(''); }}
                        style={{ marginTop: 14, padding: '8px 18px', background: '#4f46e5', color: 'white', border: 'none', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                        Clear Filters
                      </button>
                    )}
                  </div>
                          ...
                          >
                            ...
                            <td style={{ padding: '12px 16px' }}>
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                                ...
                                {(r.status === 'interview_sent' || r.status === 'approved') && (
                                  <span style={{ fontSize: 10, color: '#4f46e5', fontWeight: 500 }}>✉ Email sent</span>
                                )}
                                {r.status === 'rejected' && (
                            ...
                            <td style={{ padding: '12px 16px' }} onClick={e => e.stopPropagation()}>
                              <button
                                ...
                                onClick={() => setRejectModal({ id: r.id, name: r.candidate_name, email: r.candidate_email })}
                                style={{ padding: '5px 14px', background: '#fef2f2', color: r.status === 'rejected' ? '#94a3b8' : '#b91c1c', border: '1px solid', borderColor: r.status === 'rejected' ? '#e2e8f0' : '#fecaca', borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: r.status === 'rejected' ? 'default' : 'pointer' }}>
        ...
        {/* RIGHT — AI Panel */}
        {activeReferral && (
          <div style={{ width: 360, flexShrink: 0, background: 'white', borderRadius: 10, border: '1px solid #e2e8f0', overflow: 'hidden', position: 'sticky', top: 24, animation: 'rr-right .2s ease' }}>
            <div style={{ padding: '14px 18px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {activeReferral.status !== 'rejected' && (
                  <button onClick={() => setRejectModal({ id: activeReferral.id, name: activeReferral.candidate_name, email: activeReferral.candidate_email })}
                    style={{ padding: '5px 12px', background: '#fef2f2', color: '#b91c1c', border: '1px solid #fecaca', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                    Reject
                  </button>
                )}
            ...
            <div style={{ padding: 18, maxHeight: 'calc(100vh - 180px)', overflowY: 'auto' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                <button disabled={updating === activeReferral.id || activeReferral.status === 'approved'}
                  ...
                  {updating === activeReferral.id ? 'Updating...' : activeReferral.status === 'approved' ? '✓ Approved' : 'Approve'}
                </button>
                <button disabled={activeReferral.status === 'rejected'}
                  onClick={() => setRejectModal({ id: activeReferral.id, name: activeReferral.candidate_name, email: activeReferral.candidate_email })}
                  style={{ flex: 1, padding: '9px', background: '#fef2f2', color: activeReferral.status === 'rejected' ? '#94a3b8' : '#b91c1c', border: '1px solid', borderColor: activeReferral.status === 'rejected' ? '#e2e8f0' : '#fecaca', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: activeReferral.status === 'rejected' ? 'default' : 'pointer' }}>
                  {activeReferral.status === 'rejected' ? '✗ Rejected' : 'Reject'}
                </button>
              </div>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <button disabled={analyzing} onClick={() => appData ? handleAnalyze(undefined) : fileInputRef.current?.click()}
                  style={{ width: '100%', padding: '9px 14px', background: '#eef2ff', color: '#4f46e5', border: '1px solid #e0e7ff', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: analyzing ? 'not-allowed' : 'pointer', opacity: analyzing ? 0.7 : 1 }}>
                ...
                <>
                  ...
                  {appData.resume_url && (
                    <a href={appData.resume_url} target="_blank" rel="noreferrer"
                      style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 14px', background: '#eef2ff', border: '1px solid #e0e7ff', borderRadius: 8, textDecoration: 'none', color: '#4f46e5', fontSize: 13, fontWeight: 600 }}>
                      View Full Resume
                    </a>
                  )}
                </>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={submitReview}
                  style={{ flex: 1, padding: '10px', background: '#4f46e5', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Submit Review</button>
              </div>
            </div>
          </div>
        </div>
